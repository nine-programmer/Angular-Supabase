...
| อัปโหลดไฟล์ / รูป      | `src/server/services/storage-server.service.ts`                                                                                                  | browser ส่ง multipart มาที่ `POST /api/<feature>/:id/files` (`multer` memoryStorage, จำกัดขนาด/ชนิดตาม SPEC 2.4 เช่น 5 MB รูปเท่านั้น) → server เก็บใน bucket **private** ด้วย `service_role` → คืน signed URL อายุ 1 ชั่วโมงให้ browser แสดง; ชื่อ bucket เป็นค่าคงที่ในโค้ด ไม่ใช่ env                                                                                                                                                                                                                      |
