...
| M15 | Database hosting: **Supabase cloud** or **PostgreSQL + PostgREST on the user's own VPS** — plus the project slug you propose (English kebab-case from the system name, e.g. `barber-queue`) | ✗ — **always the last question of the interview**, asked alone in its own turn after every other item is settled and before step 3; the user picks the hosting, you propose the slug and they confirm it. Both go into SYSTEM_SPEC 2.1; Task 1 sets the database up from them (README → "ตั้งค่าฐานข้อมูล" แบบ A / แบบ B) |
