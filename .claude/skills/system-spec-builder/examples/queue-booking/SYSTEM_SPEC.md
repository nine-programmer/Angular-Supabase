...
| R2 | เปลี่ยนสถานะได้ตามแผนผังใน 1.5 เท่านั้น และจดเวลาที่ถูกเรียก | Postgres function `set_booking_status(id, new_status)`: ตรวจสถานะเดิม ถ้าไม่ตรงแผนผัง `RAISE EXCEPTION 'ข้อความไทย' USING ERRCODE = 'P0409'` (→ 409); ตั้ง `called_at = now()` เมื่อเข้า `called`; คืนแถว |
...
- Stack: มาตรฐานตาม `docs/ARCHITECTURE.md` เวอร์ชัน template 1.8
...
| PATCH | /api/bookings/:id/status | body `{ status }` → `set_booking_status()`; สถานะเดิมไม่ตรงแผนผัง → 409 (`P0409` ผ่าน `api-error.ts`) | R2, R4 |

รูปแบบตอบกลับ: ทุก endpoint ตอบ JSON เป็นแถวตาม `Tables<'services'>` / `Tables<'bookings'>` เว้นแต่ระบุเพิ่มในช่อง "ทำอะไร"; request body ตรวจด้วย zod schema และ dto ประกาศใน `shared/dto/`; error ตอบ HTTP status + `{ error: "ข้อความไทย" }`
