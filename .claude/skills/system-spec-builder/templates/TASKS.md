...
- ทำ: (1) **ตรวจความพร้อม** — `node -v` ต้องเข้าเงื่อนไข `engines` ใน `package.json` ถ้าไม่ผ่านให้บอกผู้ใช้สลับเวอร์ชัน (เช่น `nvm use <version>`) แล้ว `npm install` (2) **ตั้งค่าฐานข้อมูลตาม SPEC 2.1** — agent เปิด README → "ตั้งค่าฐานข้อมูล" แล้ว**พาผู้ใช้ทำทีละขั้นโดยยกข้อความจาก README เท่านั้น ห้ามคิดหรือเดาขั้นตอนเอง** และห้ามอ่าน `.env` (ผู้ใช้เป็นคนใส่ค่าลับเอง agent บอกได้แค่ว่าค่าแต่ละตัวเอามาจากไหนตาม comment ใน `.env.example`) [เลือกเขียนเฉพาะสายที่ SPEC 2.1 ระบุ — **สาย Supabase cloud (แบบ A)**: ผู้ใช้สร้างโปรเจกต์ที่ supabase.com ชื่อ `[project-name]` และจดรหัสผ่านฐานข้อมูล → `cp .env.example .env` → ผู้ใช้ใส่ `SUPABASE_URL` และ `SUPABASE_SERVICE_ROLE_KEY` → ผู้ใช้รัน `npx supabase login` (เปิด browser) → agent รัน `npm run db:link -- --project-ref <ref>` (ถามรหัสผ่านฐานข้อมูล) → `npm run db:push` → `npm run db:types` / **สาย VPS ของตัวเอง (แบบ B)**: ผู้ใช้ทำบน server ตามข้อ 1–5 ของแบบ B — ติดตั้ง PostgreSQL + สร้างฐานข้อมูลชื่อ `[project_name]` → ใส่ `DATABASE_URL` ใน `.env` → agent รัน `npm run db:push:url` → `npm run db:types:url` → ผู้ใช้ตั้งรหัส `authenticator`, ตั้ง PostgREST config, สร้าง JWT ตามคำสั่งในข้อ 4, ตั้ง reverse proxy, แล้วใส่ `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` ใน `.env`] → `npm start` เปิด `/api/health` ต้องได้ `{ ok: true }` ถ้าไม่ได้ agent อ่านข้อความ `error` แล้วพาแก้ตาม README → "ปัญหาที่พบบ่อยตอนตั้งค่า" จนผ่าน (3) **ตั้งชื่อ `[project-name]`** — แก้ `name` ใน package.json, เปลี่ยน **key ของ project** ใน angular.json จาก `angular-supabase` เป็น `[project-name]` เพราะ `outputPath` อิงชื่อนั้น, เปลี่ยน script `serve:ssr:angular-supabase` เป็น `serve:ssr:[project-name]` และ path `dist/[project-name]/...`, และ `project_id` ใน `supabase/config.toml` (4) **หน้าแรก** — หน้า `/` แสดงชื่อระบบเฉยๆ (แก้ `title` ใน `src/app/app.ts`, ข้อความใน `src/app/app.html` ซึ่งเป็น placeholder ของ template และ `<title>` ใน `src/index.html`) — โครง server (`src/server/env.ts`, `supabase.ts`, `api.ts`, `routes/health.routes.ts`, `services/health-server.service.ts`), interceptor ตอน SSR ทั้ง 2 ตัว (`src/app/core/api-origin.interceptor.ts`, `ssr-cookie.interceptor.ts`), `api-error.ts`, `thai-date.ts` และ `provideHttpClient` ทั้งสองฝั่งมากับ template แล้ว ไม่ต้องสร้างใหม่
...
### [ ] Task [N]: ปิดงาน
- ทำ: ไล่เช็ค SPEC 1.8 ทุกข้อ; ทุกหน้าที่ 375px; ทุกหน้าใช้ token/pattern ตาม `docs/DESIGN.md` (ไม่มีสีหรือปุ่มนอกระบบ); error ทุกจุดแสดงข้อความไทย; เขียน README.md (ฐานข้อมูลอยู่ที่ไหนตาม SPEC 2.1 — Supabase cloud หรือ VPS — และวิธีตั้งค่า `.env`, วิธีรัน, วิธี deploy (build `npm run build`, start `npm run serve:ssr:[project-name]`, Node เวอร์ชันตาม `engines` ใน package.json), ตัวแปร .env — ย้ำว่า `NG_ALLOWED_HOSTS` ต้องเป็นโดเมนจริงก่อน deploy ไม่งั้นทุกหน้าได้ 400)
