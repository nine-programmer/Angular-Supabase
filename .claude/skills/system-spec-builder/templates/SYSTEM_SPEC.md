...
- เข้าสู่ระบบ: [ไม่มี / รหัสผ่านเดียวของร้าน (ไม่มีตารางผู้ใช้) / ชื่อผู้ใช้+รหัสผ่าน / OTP ทาง ... — ตามคำตอบ interview]
...
[ทุกค่าที่ UI คำนวณให้เลือก (ช่องเวลา ราคา ตัวเลือกที่ว่าง) ต้องมี R ให้ server/DB ตรวจซ้ำ — ตัวอย่าง: "R[n]: server ปฏิเสธ start_time ที่อยู่นอกเวลาเปิด-ปิดหรือไม่ตรงรอบ slot_duration_min แม้ UI จะไม่แสดงให้เลือก"]
| # | กติกา | บังคับที่ |
|---|---|---|
...
- ชื่อโปรเจกต์ (slug): `[project-name]` — ใช้เป็น `name` ใน package.json, script `serve:ssr:[project-name]` และ path `dist/[project-name]/` และเป็นชื่อโปรเจกต์ Supabase (cloud) หรือชื่อฐานข้อมูล `[project_name]` (VPS) [ถ้าลูกค้ามีโปรเจกต์ Supabase / ฐานข้อมูลอยู่แล้ว ระบุชื่อจริงตรงนี้แทน]
- Deploy: [Render / host Node อื่น / เซิร์ฟเวอร์ตัวเอง]

### 2.2 API ที่ต้องมี [LOCKED]
| Method | Path | ทำอะไร | กติกาที่เกี่ยว |
|---|---|---|---|
| GET | /api/health | ตอบ `{ ok: true, count: N }` — N = จำนวนแถวในตารางหลัก (ตรวจว่าเชื่อม DB ได้; มากับ template แล้ว `count` เพิ่มใน Task ฐานข้อมูล — ก่อนหน้านั้นตอบ `{ ok: true }`) | — |
| GET | /api/[resource] | [ ] | [ ] |

รูปแบบ request/response: request body ทุกตัวตรวจด้วย zod schema ใน `shared/dto/` (ตามฟิลด์ บังคับ/ห้ามซ้ำ/ช่วงค่าใน 1.5); ทุก endpoint ตอบ JSON เป็นแถวตาม `Tables<'[table]'>` ของตารางนั้น (dto re-export type นั้น ไม่เขียน zod ซ้ำ — AGENTS → API Layer) เว้นแต่ระบุเพิ่มในช่อง "ทำอะไร" (เช่น field คำนวณ หรือ join ชื่อจากตารางอื่น); error ตอบ HTTP status + `{ error: "ข้อความไทย" }` ตามความหมายของ status และการแปลง error ของ DB ใน `AGENTS.md` → API Layer — ไม่ต้องนิยามซ้ำในเอกสารนี้ (ในตาราง 2.2 อ้างแค่ว่ากรณีไหนได้ 400/401/403/404/409/413/429); endpoint ที่คืนรายการต้องระบุลำดับ (`.order()`)

### 2.3 ฟีเจอร์ → ไฟล์
| ฟีเจอร์ | feature folder (`src/app/features/`) + หน้า | API ที่หน้าใช้ | server (`routes/`, `services/`) | shared (`dto/`, `enums/`) |
|---|---|---|---|---|
| F1 | `[name]/` หน้า `[path]` (`[name].page.ts + .page.html`) | [ ] | `[name].routes.ts`, `[name]-server.service.ts` | `[name].dto.ts` |
| login (ถ้ามี) | `auth/` หน้า `[path ของหน้า login]` (`pages/login.page.ts + .page.html`) | `/api/auth/login`, `/api/auth/me`, `/api/auth/logout` [+ วิธีอื่นของระบบนี้] | `auth.routes.ts` [+ `auth-server.service.ts` เมื่อมีตารางผู้ใช้] + `auth.middleware.ts`, `src/app/core/auth.guard.ts` (ชื่อชุดเดียวตาม `docs/EXTENSION_POINTS.md` แถว login ห้ามเปลี่ยน) | `auth.dto.ts` |
...
- [rate limit ของ endpoint สาธารณะ: login N ครั้ง/M นาที, จอง N ครั้ง/M นาที ต่อ IP — ตามคำถาม interview; ถ้าไม่มี endpoint สาธารณะที่เขียนข้อมูล เขียนว่า "ไม่มี"]
