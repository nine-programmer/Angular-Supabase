...
- agent ที่แก้ไฟล์ใน repo ได้ (Claude Code, Cursor) ให้แก้ไฟล์ตรง; ถ้าเป็นเว็บแชทที่ไม่เห็น repo ผู้ใช้ต้องวางไฟล์ 4 ไฟล์ (`AGENTS.md`, `docs/ARCHITECTURE.md`, เอกสารนี้, `docs/TASKS.md`) และ agent ส่งไฟล์เต็มทุกครั้ง
...
รูปแบบ request/response: request body ทุกตัวตรวจด้วย zod schema ใน `shared/dto/` (ตามฟิลด์ บังคับ/ห้ามซ้ำ/ช่วงค่าใน 1.5); ทุก endpoint ตอบ JSON เป็นแถวตาม `Tables<'[table]'>` ของตารางนั้น เว้นแต่ระบุเพิ่มในช่อง "ทำอะไร" (เช่น field คำนวณ หรือ join ชื่อจากตารางอื่น); error ตอบ HTTP status + `{ error: "ข้อความไทย" }`
