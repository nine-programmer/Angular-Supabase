...
6. Never read `.env` with any tool (Read, `cat`, `Get-Content`, `grep`, `node -e`, ...) — `.env.example` is the only env file an agent opens. A new variable is added to `.env.example` (name + comment) and the user is told to put the real value in `.env` themselves; when a step needs a value from `.env` (a project ref, a password), ask the user for it instead of reading the file. Secrets never go into `environment*.ts`.
7. Schema changes are migration files only (`npm run db:migration` → `db:push` → `db:types`; on a self-hosted database `db:push:url` / `db:types:url` replace the last two); never rename a migration, never edit one after `db:push` (add a new file), never delete `*_roles.sql` / `*_health.sql`, never hand-write `database.types.ts`.
...
10. Do not modify `docs/ARCHITECTURE.md` inside a customer project, and do not add a top-level folder outside its section 3 without asking (folders listed in `docs/EXTENSION_POINTS.md` are pre-approved).
...
13. On every screen: tokens and patterns from `docs/DESIGN.md`, real icons (never emoji), Thai-capable fonts, a visible label on every input, `aria-label` on icon-only buttons, and only `<button>`/`<a>` for clickable things (contrast ratios are guidance the agent computes — see Accessibility Requirements). The placeholder home page of Task 1 exists before `docs/DESIGN.md` does; it is restyled in the Design task, so the DESIGN.md part does not apply to it.
...
- Prefer inline templates for small components — here _small_ means a child component in `components/` or `src/app/ui/`. A routed page (`*.page.ts`) always uses `templateUrl: './<name>.page.html'` from the moment it is created; do not wait for it to grow.
...
- Error mapping is shared, not per feature — `src/server/api-error.ts` ships with the template:
  ...
  - Status meanings are therefore fixed for every project — 400 input/rule, 401 not logged in, 403 role, 404 unknown id, 409 duplicate/conflict, 413 too large, 429 too many attempts — and a SPEC never redefines them. `GET /api/health` is the one exception: it is a setup diagnostic, not a resource — `{ ok: true }` (plus `count` once the database task adds it) or `{ ok: false, error }`.
...
- The template ships two migrations that every project keeps: `supabase/migrations/*_roles.sql` creates the PostgREST roles and default privileges that Supabase cloud already has (a no-op there), so the same migrations also run on a self-hosted Postgres + PostgREST; `*_health.sql` creates `public.health()` (callable only by `service_role`), which `GET /api/health` calls through `src/server/services/health-server.service.ts` to prove that `.env` and the pushed migrations work. Never delete or rename either migration. Connecting the database is **Task 1 of `docs/TASKS.md`**: the agent walks the user through README → "ตั้งค่าฐานข้อมูล" — แบบ A (Supabase cloud) or แบบ B (PostgreSQL + PostgREST on the user's VPS) as chosen in SYSTEM_SPEC 2.1 — quoting the README steps (which say who runs what) rather than inventing them, never reading `.env` (the user pastes the secrets themselves), and never guessing dashboard menus, CLI flags, or key formats. Server-admin work the README leaves out (installing PostgreSQL, a systemd unit, `createdb`) may be proposed as standard commands, clearly marked as outside the README and confirmed by the user before running. Done when `GET /api/health` returns `{ ok: true }` against the project the user named (check `supabase/.temp/project-ref` on cloud).
- Use a typed client: generate `Database` types with `npm run db:types` (or `db:types:url` when self-hosted; Supabase CLI is a devDependency, do not assume a global install) into `src/shared/types/database.types.ts`. Do NOT hand-write table types; derive row types with `Tables<'table_name'>`.
...
- Keep schema changes as Supabase CLI migration files in `supabase/migrations/` (one file per schema change; create with `npm run db:migration -- <description>`, which names it `<timestamp>_description.sql` — never rename it), pushed and typed with the command chain in hard rule 7 (`db:push:url` / `db:types:url` read `DATABASE_URL` from `.env` and reach a VPS Postgres directly). There is never a local/Docker database for development. Do not edit the database schema manually through the dashboard without a matching migration.
- Stay portable — the server must depend only on what Postgres + PostgREST provide, so a project can leave Supabase cloud for a self-hosted Postgres + PostgREST by changing `.env` alone. Never use Supabase Auth, Realtime, or Edge Functions, and never reference the `auth.` / `storage.` SQL schemas from `src/server/` or a migration. The one allowed extra is the Storage HTTP API through `supabase-js`: keep every call to it inside `src/server/services/storage-server.service.ts` so it can be swapped for disk/S3 in one file, and record the choice in SPEC 2.4. Extensions outside standard Postgres contrib (`pgcrypto`, `pg_trgm`, `citext`, ...) are not allowed, with one exception: `pg_cron`, preinstalled on Supabase cloud but not on a VPS — a SPEC that uses it must say so in 2.4 together with the fallback (the `src/server/jobs/` endpoint from `docs/EXTENSION_POINTS.md`). Anything the app needs to run lives in a migration, never only in the dashboard.
- The installed `supabase-postgres-best-practices` skill is general Postgres reference material, and this file wins wherever they differ — in particular its RLS-policy guidance: this template keeps tables deny-all behind the API. Its primary-key advice also loses: keep `uuid` primary keys as defined in `docs/ARCHITECTURE.md` section 6 (never `bigint identity`); `uuidv7()` is allowed only when the project's Postgres is 18+. Never adopt a local-database workflow (`supabase start`, `--local`, `supabase db pull` / `db diff`, `supabase db query`) or declarative schemas (`supabase/schemas/`) from any source; each migration is hand-written and pushed to the project's database (Supabase cloud, or the self-hosted Postgres via `db:push:url`).
...
- The template keeps `src/app/app.spec.ts` as a smoke test that the root component bootstraps (a deliberate exception to the rule below). Do not extend it — replace it once the project has real component specs. The template also ships `src/server/api-error.spec.ts`, `src/server/services/health-server.service.spec.ts`, and `src/shared/utils/thai-date.spec.ts`, because those files hold the mapping/date/connectivity logic every feature relies on; the interceptors are deliberately untested because they are too simple to meet the criteria below.
...
- Do NOT introduce a top-level folder or naming convention that is not listed in `docs/ARCHITECTURE.md` section 3 without asking the user first. The extension points in `docs/EXTENSION_POINTS.md` (auth, rate limit, storage, integrations, jobs, reports, SSE, `src/server/features/`) are pre-approved: use them when the SPEC calls for that capability, and note the choice in SPEC 2.3/2.4.
...
- File size: aim for under 300 lines per hand-written file (counted after `npm run format`); above 400 lines you must split it into smaller components, services, or modules. Between 300 and 400, split only when the file clearly owns more than one responsibility — do not split a cohesive file just to hit a number, and never compress code onto long lines or skip Prettier to stay under it. `.page.html` counts as a hand-written file too — above 400 lines, move self-contained blocks (a dialog, a table, a summary card) into `components/` of that feature. Exempt: generated files (`src/shared/types/database.types.ts`), migration `.sql` files (one schema change may legitimately be long), `.spec.ts` files (split only when they test more than one unit), and the design mockup `docs/design/mockup.html` (several screens in one viewable file by design).
