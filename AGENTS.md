...
3. Every table has RLS enabled and no `anon`/`authenticated` policies; authorization is enforced in `src/server/` middleware, never only in the browser.
...
5. Every API route validates its body with a `zod` schema from `src/shared/dto/`, answers `{ error: "ข้อความไทย" }` with the right status, and never leaks raw DB errors.
6. Never read `.env`; secrets never go into `environment*.ts`; `.env.example` stays complete.
7. Schema changes are migration files only (`npm run db:migration` → `db:push` → `db:types`); never rename a migration, never delete `*_health.sql`, never hand-write `database.types.ts`.
...
13. Accessibility checklist (WCAG AA), real icons (never emoji), Thai-capable fonts, and `docs/DESIGN.md` tokens/patterns on every screen.
...
- When these docs exist: read `AGENTS.md`, `docs/ARCHITECTURE.md`, and the relevant SPEC before writing code — plus `docs/DESIGN.md` before building any screen, and use its tokens and patterns exactly instead of inventing styles; work on exactly ONE task from the TASKS file at a time; when a task passes, mark it `[x]` in TASKS.md with the date (plus the session-log filename if one was written), update its header line, then follow the end-of-task steps in Working Rules and STOP — never start the next task on your own. Never change code away from a LOCKED spec item without updating the spec (and bumping its version) first.
