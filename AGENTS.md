...
- The installed `supabase` and `supabase-postgres-best-practices` skills are general reference material, and this file wins wherever they differ. In particular, ignore their advice to use a local database (`supabase start`, `--local`, `supabase db pull` / `db diff`, `supabase db query`), declarative schemas (`supabase/schemas/`), or RLS policies for `anon`/`authenticated`: this template hand-writes each migration, pushes it to the cloud project, and keeps tables deny-all behind the API.
