...
- Always communicate with the user in Thai. Code, comments, identifiers, and suggested commit messages stay in English.
- Never commit or push unprompted. Default: when a task passes and the project is a git repository (a `.git` folder exists), end your reply by asking the user to commit before the next task and suggest a one-line commit message; if there is no `.git` folder, do not mention commits at all. If the user explicitly asks you to commit — in chat, or SYSTEM_SPEC Section 0 says `การ commit: ให้ agent commit ให้` — then commit yourself: one commit per task, English message, and record the hash on the ผล: line instead of reminding.
- After every task that passes (or any substantial piece of work when no TASKS file exists), ASK the user whether to write a session log to `.sessions/YYYY-MM-DD-HHmm-<task-slug>.md`; write it only if they say yes (create the folder if missing; the folder is committed, not ignored). It is the shared memory for whichever AI tool works next, so record: what was done, files touched, decisions and workarounds with the reason, how it was tested, and what the next task is. Read the newest logs before starting work.
- End every finished task, in this order: the session-log question, the commit reminder (only if `.git` exists), then the closing line "Task นี้เสร็จสิ้นแล้วครับ จะทำ Task ถัดไปที่ session นี้เลย หรือจะเปิด session ใหม่? ถ้าเปิดใหม่ ใช้ prompt นี้ได้: `อ่าน docs/SYSTEM_SPEC.md แล้วเริ่มตาม Section 0`" — and wait. Do not recommend one option over the other, and do not begin the next task until the user tells you to.
