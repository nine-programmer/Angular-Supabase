...
- Text/background pairings meet AA contrast (4.5:1 body text, 3:1 large text and UI borders). Stay on pairings already known to pass — Tailwind's `slate-900`/`slate-600` on white and `slate-100`/`slate-400` on `slate-900`, or project tokens defined in `@theme` — and check the ratio before introducing a new pairing or a new token.
...
- `ng test` errors out when a project contains no spec at all, so the template keeps `src/app/app.spec.ts` as a deliberate exception to the rule below: it only asserts that the root component bootstraps. Do not extend it — replace it once the project has real specs. It is the ONLY spec the template ships; the interceptor and the health service are deliberately untested because they are too simple to meet the criteria below.
...
- Non-secret browser config lives in `src/environments/` only. Do NOT add the runtime config pattern from `angular-developer/references/environment-configuration.md` (fetching `/assets/config.json` during bootstrap) — it is outside this template's scope, and that reference still names the pre-v18 `src/assets/` location, which this project does not have: static files live in `public/`.
- The skills in `.claude/skills/` / `.agents/skills/` are reference material only; wherever they differ from this file or `docs/ARCHITECTURE.md`, this file wins. `angular-developer`, `angular-new-app`, and `supabase-postgres-best-practices` come from upstream and are tracked in `skills-lock.json`; `tailwind-css-patterns` (an in-house fork adapted for Angular) and `system-spec-builder` are maintained here, so they are deliberately absent from that lock file. Known differences: their file naming and one-spec-per-file scaffolding (next bullet); in `supabase-postgres-best-practices`: `bigint identity` primary keys, RLS policies and `auth.uid()`-based security (this template has no client-side auth — see Supabase), the `SECURITY DEFINER` recommendation with `revoke execute ... from service_role` (would make a function uncallable here), and the whole connection-pooling category (`conn-*.md` — the server talks to Supabase over HTTPS through `supabase-js`, there is no pool to size); in `angular-developer`: the `ng build` finish step in `SKILL.md` and `references/migrations.md` (see Working Rules — this repo finishes with `npm run format` then `npm test`), the "Karma or Vitest" test runner in `references/cli.md` (this repo is Vitest only — see Testing), `references/signal-forms.md` making Signal Forms mandatory and banning `FormGroup` (this file only prefers them), `references/naming-conventions.md` describing `shared/` as a UI toolkit (`src/shared/` here is plain TypeScript; reusable UI lives in `src/app/ui/`), and the `src/proxy.conf.json` dev proxy in `references/cli.md` (the API is served by the same SSR process). `angular-new-app` is written for `ng new` on an empty folder and does not apply to a project cloned from this template.
...
- Keep every hand-written file under 300 lines, counted AFTER running `npm run format`. If a file grows beyond that, split it into smaller components, services, or modules and import them. Never compress code onto long lines or skip Prettier to stay under the limit — the limit is about one file owning too many responsibilities, not about line wrapping. Exempt: generated files (`src/shared/types/database.types.ts`), migration `.sql` files (one schema change may legitimately be long), and `.spec.ts` files (split only when they test more than one unit).
