...
- End every finished task, in this order: the session-log question, then the closing line "Task นี้เสร็จสิ้นแล้วครับ จะทำ Task ถัดไปที่ session นี้เลย หรือจะเปิด session ใหม่? ถ้าเปิดใหม่ ใช้ prompt นี้ได้: `อ่าน docs/SYSTEM_SPEC.md แล้วเริ่มตาม Section 0`" — and wait. Do not recommend one option over the other, and do not begin the next task until the user tells you to.
